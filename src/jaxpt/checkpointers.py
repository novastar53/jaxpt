...
def save_checkpoint(m, output_dir, run_dirname, step):
  checkpoint_path = output_dir / m.name / "checkpoints" / run_dirname / f"checkpoint-{step}.pt"
  checkpoint_path.mkdir(parents=True, exist_ok=True)
  m.save_checkpoint(checkpoint_path)


def load_checkpoint(model, output_dir, config, run_dirname, step, rngs):
  checkpoint_path = output_dir / config.name / "checkpoints" / run_dirname / f"checkpoint-{step}.pt"
  m = model.from_checkpoint(checkpoint_path, rngs, config)
  return m


def load_checkpoint_from_gcloud(model, config, output_dir, bucket_name, run_dirname, step, rngs):

...
from jaxpt.modules.moe import SoftMOE
...
class Tiny_MoE_3_Config(Config):
    name: str = "Tiny_MoE_3"
...
class GLU_Block(nnx.Module):
    def __init__(
        self, config: Tiny_MoE_3_Config, rope_omega: nnx.Variable, rngs: nnx.Rngs
    ) -> None:
...
class Tiny_MoE_3(nnx.Module):
    def __init__(self, config: Tiny_MoE_3_Config, rngs: nnx.Rngs):
        self.config = config
        self.wte = nnx.Embed(
            config.vocab_size,
            config.n_embed,
    ...
    @staticmethod
    def from_checkpoint(
        fpath: str, rngs: nnx.Rngs, config: Optional[Tiny_MoE_3_Config] = None, sharding: Optional[jax.sharding.NamedSharding] = None
    ):
    
        default = jax.random.key(1337)
        gate_noise = jax.random.key(42)
        rngs = nnx.Rngs(default=default, gate_noise=gate_noise)
        config = config if config else Tiny_MoE_3_Config()
        abstract_model = nnx.eval_shape( 
            lambda: Tiny_MoE_3(config=config, rngs=nnx.Rngs(default=default, gate_noise=gate_noise))
        )

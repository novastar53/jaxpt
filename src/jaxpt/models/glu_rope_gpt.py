...
class GLU_RoPE_GPTConfig(Config):
...
class Block(nnx.Module):
    def __init__(self, config: GLU_RoPE_GPTConfig, rope_omega: nnx.Variable, rngs: nnx.Rngs):
...
class GLU_RoPE_GPT(nnx.Module):
    def __init__(self, config: GLU_RoPE_GPTConfig, rngs: nnx.Rngs):
        self.config = config
        self.wte = nnx.Embed(
            config.vocab_size,
            config.n_embed,
    ...
    @staticmethod
    def from_checkpoint(fpath: str, rngs: nnx.Rngs, config=Optional[GLU_RoPE_GPTConfig]):
        config = config if config else GLU_RoPE_GPTConfig()

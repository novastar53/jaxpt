...
class GQA_GLU_RoPE_GPTConfig(Config):
    ...
    n_kv_head: int = 4 # number of kv heads
...
class Block(nnx.Module):
    def __init__(self, config: GQA_GLU_RoPE_GPTConfig, rope_omega: nnx.Variable, rngs: nnx.Rngs):
...
class GQA_GLU_RoPE_GPT(nnx.Module):
    def __init__(self, config: GQA_GLU_RoPE_GPTConfig, rngs: nnx.Rngs):
        self.config = config
        self.wte = nnx.Embed(
            config.vocab_size,
            config.n_embed,
        ...
        # pre-calculate the GLU_RoPE thetas
    ...
    @staticmethod
    def from_checkpoint(fpath: str, rngs: nnx.Rngs, config=Optional[GQA_GLU_RoPE_GPTConfig]):
        config = config if config else GQA_GLU_RoPE_GPTConfig()
        model = GLU_RoPE_GPT(config=config, rngs=rngs)

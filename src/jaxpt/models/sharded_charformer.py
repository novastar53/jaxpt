...
fmt = logging.Formatter(
    "%(relativeCreated)6.0fms - %(levelname)s - %(message)s"
)
...
lecun_init = nnx.with_partitioning(
    nnx.initializers.lecun_normal(dtype=DTYPE), (None, "model")
)
zeros_init = nnx.with_partitioning(nnx.initializers.zeros_init(), ("model",))
...
def attention_with_masking(_Q, _K, _V, seq_pos=BLOCK_SIZE):
    ...
    kv_segment_id = jnp.ones((1, KV_SIZE), jnp.int32) * jnp.expand_dims(
        jnp.arange(KV_SIZE) <= seq_pos, axis=0
    )

    segment_ids = pallas_attention.SegmentIds(
        q=query_segment_id, kv=kv_segment_id
    )
...
class Attention(nnx.Module):
    def __init__(self, rngs: nnx.Rngs, dtype=jnp.float32):
        ...
        self.key_cache = nnx.Variable(
            jnp.zeros((1, BLOCK_SIZE, NUM_HEADS, HEAD_DIM))
        )
        self.value_cache = nnx.Variable(
            jnp.zeros((1, BLOCK_SIZE, NUM_HEADS, HEAD_DIM))
        )
    ...
    def __call__(self, x, pos=0, use_cache=False):
        ...
        if use_cache:
            ...
            output = attention_with_masking(
                q, self.key_cache, self.value_cache, seq_pos=pos + T
            )
        else:
...
class Model(nnx.Module):
    def __init__(self, rngs: nnx.Rngs, dtype=jnp.float32):
        ...
        self.layers = [Block(rngs, dtype) for _ in range(NUM_LAYERS)]
...
def train():
    mesh = jax.sharding.Mesh(
        np.reshape(jax.devices(), (DATA_DIMS, MODEL_DIMS)), ["data", "model"]
    )
    ...
    with mesh:
        ...
        with jax.profiler.trace("/home/vikram/jaxpt/tensorboard"):
            for step in range(100):
                ...
                example = next(ds)
                # print(example['text'])
                inputs, labels = prepare_train_batch(
                    example["text"].numpy(), BLOCK_SIZE
                )
                ...
                if step % 10 == 0:
                    ...
                    per_device_tflops_per_second = (
                        flops_per_device * 10 / 1e12 / time_elapsed_seconds
                    )
                    logger.info(
                        f"step: {step}, loss: {loss}, time_elapsed: {time_elapsed_seconds:.4f}s, tflops/device/s: {per_device_tflops_per_second:.4f}"
                    )
                step += 1
